...
- [ ] 사용자로부터 당첨 번호 6개와 보너스 번호 1개를 입력받아, 당첨 여부를 판별한다.
  ...
  - [ ] 모든 번호는 중복되지 않는 6개의 정수이며, 쉼표를 기준으로 구분한다.
    - [ ] `validate()` in `parsing()`
    - ### ㄴ `테스트`
      - [X] 6개의 중복되지 않는 숫자를 쉼표로 구분해서 입력 - `기능`
  ...
  - [ ] 6개의 로또 번호로 numbers List 생성한다.
  - [ ] numbers List 로 Lotto 를 생성한다. 
    - [ ] `class Lotto`
    - [ ] `validate()` int `constructor()`
    - ### ㄴ `테스트`
      - [X] 중복되지 않는 숫자 6개일 경우 - `기능`
      - [X] 중복된 숫자가 있을 경우 - `예외`
      - [X] 숫자가 6개 미만 - `예외`
      - [X] 숫자가 6개 초과 - `예외`
      - [X] 숫자가 범위를 벗어남 - `예외`
  - [ ] 1개의 보너스 번호를 입력받는다.
  - [ ] 보너스 번호는 당첨 번호들과 중복되지 않는다.
  - [ ] `class Winning`
