...
- [X] 실패했지만 의미있던 도전 정리
  ...
  - 통합 테스트 구현 및 TDD
- [X] 다음에 다르게 접근해보고 싶은 부분 정리
  - `완벽한` 테스트 설계에 대한 집착을 버리기
- [X] README.md 상세히 작성
...
- [X] 판별된 당첨 결과와 로또 구매 금액을 기반으로 수익률을 출력한다.
  ...
  - [X] `calcProfit(int amount, int totalPrize)`
    ...
    - Statistics? 최종 수익률 계산에 구매 금액을 활용하는게 통계이므로.
    - 결국 둘 중 하나의 클래스의 필드로만 존재하는 것이 맞다면 
    - 먼저 받아서 사용하는 서비스가 보유하는게?
    - 통계는 메서드의 매개변수로 받아서 계산에 활용.
...
- [X] 사용자가 잘못된 값을 입력할 경우, `IllegalArgumentException`을 발생시키고,  
  ...
  - [X] `예외`
  - [X] 숫자 대신 `문자`를 입력(당첨 번호 입력에서의 구분자 `,` 제외)
    - [X] 공백 입력
    - [X] 소수(소수점) 입력
    - [X] 음수(`-`기호) 입력
  - [X] 중복된 숫자 입력
  - [X] 범위 초과 숫자 입력
  - [X] 1000으로 나누어 떨어지지 않는 구입 금액 입력
  - [X] `재입력`
  - [X] 예외가 발생한 위치, 해당 메소드를 그대로 재실행
  
    - [X] ### [❗잘못된 입력에 대한 재시도, 반복 처리는 어떻게?](#-생각할-부분)
...
- [X] 통합적인 기능에 대한 통합 테스트를 통해 의도한 대로 정확하게 동작하는 구현을 확인한다.
